...
if (empty($errors)) {
    if ($_POST['category']== "clothing" ) {
        if ($size != "") {
            $statement = $pdo->prepare("UPDATE products SET size = :size WHERE id_product = :id");
    ...
    if (!empty($imagePath)) {
        $statement = $pdo->prepare("UPDATE products SET image = :image WHERE id_product = :id");
        $statement->bindValue(':id', $id);
        $statement->bindValue(':image', $imagePath);
        $statement->execute();
    }

    $statement = $pdo->prepare("UPDATE products SET category = :category ,nom = :nom, color = :color , quantity = :quantity , description =:description ,
                ...
                <div class="container mt-5">
                    <form action="" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form">
                            ...
                            if ($product['color'] != " ") {
                                echo '<h6 style="display:inline;" >you the choose  ' . $color . '  color<h6>';
                            }
                            ?>
                        ...
                        if ($product['category'] == 'clothing' && $product['size'] != " ") {
                            echo '<h6>you choose the '  .$size.' size<h6>
                            <div class="form">
                                <select name="size" id="">
